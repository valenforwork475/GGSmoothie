...
(() => {
  ...
  function boot(runtime, doc = document) {
    ...
    fetch(location.href).then((res) => res.ok ? res.text() : "").then((t) => {
      // A bundled standalone page stores the original document as a JSON
      // string. Re-parsing that response would treat JSON-escaped quotes as
      // literal attribute characters and corrupt every sc-if/sc-for binding.
      if (t.includes('type="__bundler/template"')) return;
